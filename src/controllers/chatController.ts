...
// 2. Mendapatkan Daftar Chat / Inbox Pembeli & Penjual
export const getUserChatListController = async (
  ...
  try {
    ...
    const [userRows]: any = await db.query('SELECT role FROM users WHERE id = ?', [user_id]);
    ...
    if (role === 'penjual') {
      // Jika penjual, cari shop_id dari toko miliknya
      const [tokoRows]: any = await db.query('SELECT shop_id FROM toko WHERE user_id = ?', [user_id]);
      if (tokoRows.length === 0) {
        return res.status(200).json([]);
      }
      const shopId = tokoRows[0].shop_id;

      // Query room chat yang terikat dengan toko ini, tampilkan nama pembeli sebagai shop_name
      const query = `
              SELECT 
                  cr.room_id, cr.shop_id, u.nama_lengkap AS shop_name, NULL AS shop_src,
          ...
          `;
      const [rows]: any = await db.query(query, [user_id, shopId]);
      return res.status(200).json(rows);
    } else {
      // Jika pembeli / role lain, cari room chat berdasarkan user_id (pembeli)
      const query = `
              SELECT 
...
export const getChatMessagesController = async (
  ...
  try {
    // 1. Ambil informasi detail toko dan pembeli yang terikat dengan room ini
    const roomQuery = `
      SELECT cr.room_id, cr.shop_id, t.shop_name, u.nama_lengkap AS buyer_name
      FROM chat_rooms cr
      JOIN toko t ON cr.shop_id = t.shop_id
      JOIN users u ON cr.user_id = u.id
      WHERE cr.room_id = ?
    ...
    return res.status(200).json({
      ...
      buyer_name: roomRows[0].buyer_name,
