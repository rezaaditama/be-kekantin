...
export const createOrder = async (req: Request, res: Response) => {
    ...
    const { order_id, user_id, pickup_time, total_harga, payment_method, items } = req.body;
    ...
    try {
        ...
        await connection.query(
            ...
            [order_id, user_id, pickup_time, total_harga, payment_method, initialStatus]
        );
...
export const getOrderByUserIdController = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [rows] = await db.query(
            `SELECT o.*, oi.product_id, m.product_name, m.product_price, oi.note, oi.qty, oi.subtotal, m.product_path, t.shop_name FROM orders o LEFT JOIN order_items oi ON o.order_id = oi.order_id LEFT JOIN menu m ON oi.product_id = m.product_id LEFT JOIN toko t ON m.shop_id = t.shop_id WHERE o.user_id = ? ORDER BY o.created_at DESC`, [user_id]
        );
...
export const handleMidtransNotification = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await db.query(
            "UPDATE orders SET status_pembayaran = ? WHERE order_id = ?", [statusFinal, orderId]
        )
