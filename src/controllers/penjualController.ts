...
export const getBerandaPenjual = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [tokoRows]: any = await db.query('SELECT * FROM toko WHERE user_id = ?', [user_id]);
...
export const getPesananAktif = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [toko]: any = await db.query('SELECT shop_id FROM toko WHERE user_id = ?', [user_id]);
        if (toko.length === 0) return res.status(404).json({ message: "Toko tidak ditemukan" });
        const shopId = toko[0].shop_id;

        // Ambil list lengkap pesanan aktif
...
export const updateStatusSelesai = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: "Gagal memperbarui: Order ID tidak ditemukan" });
        }
...
export const updateProfilePenjual = async (req: Request, res: Response) => {
    ...
    try {
        // Update table user
        const queryUser = "UPDATE users SET nama_lengkap = ?, email = ?, nomor_telepon = ? WHERE id = ?";
